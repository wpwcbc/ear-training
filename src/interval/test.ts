...
const buildMode1Queue = (config: Mode1Config): Mode1QueueEvent[] => {
...
const buildMode2Queue = (config: Mode2Config): Mode2QueueEvent[] => {
...
const buildQuestion = (event: QueueEvent): Question => {
	let chordRootMidi = 0;
	let chordTonicName = "";
	const minRootMidi =
...
const handleAnswer = (selectedInterval: string): void => {
	...
	if (selectedInterval === state.currentQuestion.intervalLabel) {
		...
		const chordDegree =
			state.currentQuestion.mode === "mode1"
...
export const startTest = (config: Mode1Config | Mode2Config, mode: ActiveMode): void => {
...
export const rerunMode1 = (shiftSemis: number): void => {
	if (!state.lastMode1Config) {
		return;
	}
	const updatedKey = theory.transposeKeyCenter(
		state.lastMode1Config.keyCenter,
		shiftSemis,
	);
