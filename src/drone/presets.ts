...
const normalizePreset = (
	...
	for (const chord of chordsInput) {
		...
		const durationModeRaw = String(
			(chord as ProgressionChordPreset).durationMode || "",
		);
		const durationMode: DurationMode = durationModeRaw === "/" ? "/" : "x";
		...
		if (
			!chordDegree ||
			!CHORD_QUALITIES.includes(quality as ChordQuality) ||
			!scaleId
		) {
			return null;
		}
...
export const initProgressions = (onApplied?: () => void): void => {
