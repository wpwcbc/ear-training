...
import type { AudioState, BreathingNote } from "./types.js";
...
export const isAudioReady = (): boolean => audioState.audioReady;

export const stopChord = (): void => {
...
export const stopRootSustain = (): void => {
...
export const playChord = (notes: string[], velocities: number[]): void => {
	if (!audioState.audioReady || !audioState.chordSynth) {
		return;
	}
	stopChord();
...
export const playRootSustain = (note: string, velocity: number): void => {
	if (!audioState.audioReady || !audioState.rootSynth) {
		return;
	}
	const now = Tone.now();
...
const scheduleAmbientPulse = (notes: BreathingNote[]): void => {
	...
	const duration =
		...
		Math.random() *
			Math.max(
				0,
				AMBIENCE_SETTINGS.breathing.pulseDurationMax -
					AMBIENCE_SETTINGS.breathing.pulseDurationMin,
			);
	const percent =
		...
		Math.random() *
			Math.max(
				0,
				AMBIENCE_SETTINGS.breathing.velocityPercentMax -
					AMBIENCE_SETTINGS.breathing.velocityPercentMin,
			);
	const velocity = Math.max(0, Math.min(1, pick.maxVelocity * percent));
	audioState.ambientSynth.triggerAttackRelease(
		pick.note,
		duration,
		undefined,
		velocity,
	...
	const delay =
		...
		Math.random() *
			Math.max(
				0,
				AMBIENCE_SETTINGS.breathing.pulseDelayMaxMs -
					AMBIENCE_SETTINGS.breathing.pulseDelayMinMs,
			);
	state.ambientTimer = window.setTimeout(
		() => scheduleAmbientPulse(notes),
		delay,
	);
};

export const startAmbient = (notes: BreathingNote[]): void => {
	stopAmbient();
