...
import { addProgressionRow, getAmbienceMode, getMode, getPlaybackMode, loadDefaultProgression, markProgressionCustom, readMode1Config, readMode2Config, renderMode2Rows, showModePanels, } from "./setup.js";
import { nextChord, readBeatsPerBar, readBpm, resetExerciseDisplay, scheduleAutoAdvance, setDroneRunning, setStatus, startDrone, stopDrone, updateNextChordButton, updatePlaybackUI, } from "./exercise.js";
const normalizeKeyLabel = (key, code) => {
    if (code === "Space") {
        return "Space";
    }
    if (key === " ") {
...
const applyPlaybackParam = () => {
    ...
    const radio = document.querySelector(`input[name='playbackMode'][value='${value}']`);
    if (radio) {
        radio.checked = true;
    }
};
...
const updateModeConfigFromLast = () => {
    if (state.activeMode === "mode1" && state.lastMode1Config) {
        ...
        document.getElementById("mode2Questions").value =
...
const syncAutoAdvanceState = (validate = false) => {
...
const playbackRadios = document.querySelectorAll("input[name='playbackMode']");
