import { PROGRESSION_PRESETS, PROGRESSION_CATEGORIES, } from "../core/progressions.js";
...
import { applyProgressionPreset, buildCustomPresetFromSetup, setSaveError, } from "./setup.js";
let onPresetApplied;
const normalizePreset = (raw, forcedCategory) => {
    ...
    for (const chord of chordsInput) {
        ...
        const scaleId = resolveScaleId(String(chord.scaleId || "").trim());
        const durationModeRaw = String(chord.durationMode || "");
        const durationMode = durationModeRaw === "/" ? "/" : "x";
        const rawDurationValue = Number(chord.durationValue);
        const durationValue = durationMode === "/"
            ...
            : Number.isFinite(rawDurationValue) && rawDurationValue >= 1
                ...
                : 1;
        if (!chordDegree ||
            !CHORD_QUALITIES.includes(quality) ||
            !scaleId) {
            return null;
        }
        try {
...
const handleDeleteCustom = (id) => {
    state.customProgressions = state.customProgressions.filter((preset) => preset.id !== id);
    saveCustomProgressions(state.customProgressions);
    renderProgressionList();
};
const handleImportCustom = async (input) => {
    ...
    try {
        ...
        imported.forEach((preset) => {
            const existingIndex = state.customProgressions.findIndex((item) => item.name === preset.name);
            if (existingIndex >= 0) {
...
const loadTuneProgressions = async () => {
    try {
    ...
    PROGRESSION_CATEGORIES.forEach((category) => {
        ...
        if (!matches.length) {
            ...
            matches.forEach((preset) => {
                ...
                btn.addEventListener("click", () => applyProgressionPreset(preset, onPresetApplied));
...
const saveProgressionFromSetup = () => {
    setSaveError();
...
export const initProgressions = (onApplied) => {
    ...
    dom.elSaveProgressionName.addEventListener("input", () => setSaveError());
};
