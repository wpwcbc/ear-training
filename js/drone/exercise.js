...
export const resetExerciseDisplay = () => {
...
const buildChordNotes = (event, minRootMidi) => {
    let chordRootMidi = 0;
    let chordTonicName = "";
    if (event.mode === "mode1") {
        const keyCenter = event.keyCenter || "C";
        chordRootMidi = theory.computeChordRootMidiFromKey(keyCenter, event.chordDegree || "1", 1);
        ...
        chordRootMidi = theory.computeChordRootMidiFromNote(event.chordRoot || "C", 1);
        chordRootMidi = theory.clampRootMidiToMin(chordRootMidi, minRootMidi);
...
const getChordSymbol = (event, minRootMidi) => {
...
const buildBars = (minRootMidi) => {
    const bars = [];
    let current = { segments: [], units: 0 };
    state.eventBarMap.clear();
    const flush = () => {
...
const updateChordHighlight = (index) => {
    ...
    dom.elDroneChordList
        .querySelectorAll(".chord-pill")
        .forEach((pill) => {
        const match = Number(pill.dataset.barIndex) === activeBarIndex;
        pill.classList.toggle("is-active", match);
    });
    dom.elDroneChordList
        .querySelectorAll(".chord-segment")
        .forEach((segment) => {
        const match = Number(segment.dataset.eventIndex) === index &&
...
const playIndex = (index, minRootMidi) => {
...
export const scheduleAutoAdvance = (minRootMidi) => {
    clearAutoAdvanceTimer();
...
const runMetronomeBeat = (minRootMidi) => {
    if (!state.questionQueue.length) {
        resetExerciseDisplay();
    ...
    const { beats, rounded } = computeBeatsFromDuration(event, state.metronomeBpb);
    ...
    state.currentBarOffset =
        event.durationMode === "x"
            ? Math.min(Math.max(0, event.durationValue - 1), Math.floor(state.metronomeBeatIndex / state.metronomeBpb))
            : 0;
    updateChordHighlight(state.questionIndex);
    const chord = buildChordNotes(event, minRootMidi);
    const velocities = collectVelocitySettings();
    audio.playChordBeat(chord.notes, [
        velocities.root,
        velocities.fifth,
        velocities.seventh,
        velocities.third,
...
const buildMode1Queue = (config) => {
    ...
    for (let loopIndex = 0; loopIndex < config.loopTimes; loopIndex += 1) {
        const loopQueue = buildMode1QueueForLoop(config, loopIndex, chordIndex, totalChords);
        queue.push(...loopQueue);
        chordIndex += loopQueue.length;
    }
    return queue;
};
const buildMode1QueueForLoop = (config, loopIndex, chordOffset = 0, totalChords = config.progression.length) => {
...
const startNextLoop = (minRootMidi) => {
...
export const setDroneRunning = (running) => {
    ...
    document
        .querySelectorAll("input[name='playbackMode']")
        .forEach((input) => {
        input.disabled = running;
    });
    document
...
export const startDrone = (config, mode) => {
    if (mode === "mode1") {
        const mode1Config = config;
        ...
        state.loopIndex = 0;
        state.loopBaseConfig = null;
        state.questionQueue = buildMode2Queue(config);
    }
    state.questionIndex = 0;
    stopMetronome();
    state.roundingWarningShown = false;
...
export const nextChord = (minRootMidi) => {
    if (!state.questionQueue.length) {
        return;
    }
