...
export const renderRecords = () => {
    ...
    const renderStats = (items) => {
        ...
        const header = document.createElement("div");
        header.className = "stats-row stats-header";
        header.append(buildCell(state.statsGrouping === "quality" ? "Quality" : "Root degree", "stats-cell stats-label"), buildCell("Avg time"), buildCell("Avg attempts"), buildCell("Questions"));
        table.appendChild(header);
        const topKeyType = state.statsGrouping === "quality" ? "quality" : "degree";
        const subKeyType = state.statsGrouping === "quality" ? "degree" : "quality";
        const topKey = state.statsGrouping === "quality"
            ? (record) => record.chordQuality
            : (record) => record.chordDegree;
        const subKey = state.statsGrouping === "quality"
            ? (record) => record.chordDegree
            : (record) => record.chordQuality;
        const sortDirection = state.statsOrder === "time-desc" ? -1 : 1;
        const sortGroups = state.statsOrder === "name"
            ? (a, b) => compareByName(a, b, topKeyType)
            : (a, b) => compareByTime(a, b, sortDirection, topKeyType);
        const sortSubgroups = state.statsOrder === "name"
        ...
        groups.forEach((group) => {
            ...
            const groupLabel = state.statsGrouping === "quality"
                ? group.key
            ...
            subEntries.forEach((entry) => {
                ...
                const label = state.statsGrouping === "quality"
